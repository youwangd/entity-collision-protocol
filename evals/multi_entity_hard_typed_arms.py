"""D1 v0.4 — type-aware PRF gate A/B on multi-entity-hard.

Tests whether the type-purity gate added in
`engram.retrieval.expansion.expand_query_typed` recovers the multi-entity-hard
hit@k regression observed under plain PRF (anchor 18, §4.9).

Arms tested (single-seed default; `--seeds` for multi-seed paired-bootstrap CI):

    baseline           : no PRF, no share_prior
    prf_heur           : plain PRF, heuristic NER backend (legacy v0.3 arm)
    prf_typed_spacy    : typed PRF, spaCy backend, purity_min from CLI
    prf_typed_heur     : typed PRF, heuristic backend (purity gate inert by
                         construction — should match prf_heur, sanity check)

Output: bench/results/multi_entity_hard_typed.json + console summary.

Usage:
    python -m evals.multi_entity_hard_typed_arms \\
...
ARMS = ["baseline", "prf_heur", "prf_typed_spacy", "prf_typed_heur"]


def _build_config(arm: str, *, qe_dominance: float, type_purity_min: float) -> Config:
    cfg = Config()
    ...
    elif arm == "prf_typed_spacy":
        cfg.retrieval.entity_ner = "spacy_sm"
        cfg.retrieval.query_expansion_type_purity_min = type_purity_min
    elif arm == "prf_typed_heur":
        cfg.retrieval.entity_ner = "heuristic"
        cfg.retrieval.query_expansion_type_purity_min = type_purity_min
    return cfg
...
def _eval_arm(arm: str, ds, *, k: int, qe_dominance: float,
              type_purity_min: float) -> dict:
    with tempfile.TemporaryDirectory() as tmp:
        cfg = _build_config(arm, qe_dominance=qe_dominance,
                            type_purity_min=type_purity_min)
        cfg.path = tmp
...
def main() -> None:
    ...
    per_seed_hits = {arm: {"hit@1": [], "hit@5": [], "hit@10": []} for arm in ARMS}
    seed_results: list[dict] = []
    for s in seeds:
        cfg_fix.seed = s
        ds_s = generate_multi_entity_hard(cfg_fix)
        print(f"[meh-typed][s={s}] {len(ds_s.memories)} mem, "
              f"{len(ds_s.queries)} queries")
        base = None
        for arm in ARMS:
            t0 = time.monotonic()
            r = _eval_arm(arm, ds_s, k=args.k,
            ...
            if arm == "baseline":
                base = r
                print(f"[s={s}][{arm:18s}] h1={r['hit@1']} h5={r['hit@5']} "
                      f"h10={r['hit@10']} p50={r['p50_ms']}ms wall={wall:.1f}s")
            else:
                d1 = r["hit@1"] - base["hit@1"]
                d5 = r["hit@5"] - base["hit@5"]
                d10 = r["hit@10"] - base["hit@10"]
                print(f"[s={s}][{arm:18s}] h1={r['hit@1']} h5={r['hit@5']} "
                      f"h10={r['hit@10']} Δ1={d1:+.4f} Δ5={d5:+.4f} "
                      f"Δ10={d10:+.4f} p50={r['p50_ms']}ms wall={wall:.1f}s")
    ...
    if multi:
        ...
        print("\n[meh-typed-ci] paired-bootstrap Δ vs baseline:")
        for arm, entry in ci_block.items():
            for k_ in ("hit@1", "hit@5", "hit@10"):
                d = entry[f"d_{k_}"]
                print(f"  [{arm:18s}] Δ{k_:6s}: {d['mean']:+.4f} "
                      f"[{d['ci_lo']:+.4f}, {d['ci_hi']:+.4f}]")

    if args.out:
        Path(args.out).parent.mkdir(parents=True, exist_ok=True)
        atomic_write_json(args.out, out)
        print(f"[meh-typed] wrote {args.out}")
