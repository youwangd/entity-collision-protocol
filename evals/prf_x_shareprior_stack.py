"""§5.4 stack interaction — PRF query expansion × share_prior reranker.

Both interventions individually closed the open angles (1 and 2) on the
bridge corpus. Open question for the paper: do they STACK (additive
gains), INTERFERE (one zeroes the other), or are they REDUNDANT (the
second adds nothing on top of the first)?

Cells (4):
  - C0: baseline (no PRF, no reranker)
  - CP: PRF only (min_dominance=0.3, top_k_for_prf=10, max_entities=4)
  - CR: share_prior only (alpha=0.05, pool=20)
  - CB: both PRF and share_prior

Metrics: bridge pair_recall@5/10/20 + any_hit, plus unique-corpus
do-no-harm hit@1/hit@5. 3 seeds. ~2 minutes wall.

Driver: python -m evals.prf_x_shareprior_stack --seeds 17,42,101
Output: evals/results/prf_x_shareprior_stack.json + markdown stdout.
"""

from __future__ import annotations

import argparse
import statistics
import tempfile
import time
from pathlib import Path

from engram import Engram, Config
from engram.core.config import RetrievalConfig
from evals.share_prior_sweep import generate_bridge_corpus
from evals.entity_channel_sweep import generate_entity_corpus
from evals.query_entity_expansion import _expand_query
from evals.io_utils import atomic_write_json


def _build_engine(path: str, *, reranker: str | None,
                  share_prior_alpha: float) -> Engram:
    cfg = Config(path=path)
    cfg.security.max_events_per_minute = 0
    cfg.retrieval = RetrievalConfig(
        reranker=reranker,
        rerank_pool_size=20,
        share_prior_alpha=share_prior_alpha,
    )
    return Engram(config=cfg)


def _eval_bridge(ds, *, expand: bool, reranker: str | None,
                 alpha: float, top_k_for_prf: int, max_entities: int,
                 min_dominance: float, k_max: int = 20) -> dict:
    rows = []
    with tempfile.TemporaryDirectory() as tmp:
        eng = _build_engine(tmp, reranker=reranker, share_prior_alpha=alpha)
        try:
            for content, meta in ds.memories:
                clean = {k: v for k, v in meta.items()
                         if isinstance(v, (str, int, float, bool))}
                eng.remember(content, **clean)
            for q in ds.queries:
                first_pass = eng.recall(q.text, limit=k_max)
                if expand:
                    first_texts = [r.memory.content for r in first_pass]
                    expanded_q, chosen = _expand_query(
                        q.text, first_texts,
                        top_k_for_prf=top_k_for_prf,
                        max_entities=max_entities,
                        min_dominance=min_dominance,
                    )
                    results = (eng.recall(expanded_q, limit=k_max)
                               if chosen else first_pass)
                else:
                    results = first_pass
                texts = [r.memory.content for r in results]
                a, b = q.expected_substrings[0], q.expected_substrings[1]

                def hit(needle, top):
                    n = needle.lower()
                    return any(n in t.lower() for t in top)

                rows.append({
                    f"any@{k}": int(hit(a, texts[:k]) or hit(b, texts[:k]))
                    for k in (5, 10, 20)
                } | {
                    f"pair@{k}": int(hit(a, texts[:k]) and hit(b, texts[:k]))
                    for k in (5, 10, 20)
                })
        finally:
            eng.close()
    n = max(len(rows), 1)
    return {
        f"any_hit@{k}": sum(r[f"any@{k}"] for r in rows) / n
        for k in (5, 10, 20)
    } | {
        f"pair_recall@{k}": sum(r[f"pair@{k}"] for r in rows) / n
        for k in (5, 10, 20)
    }


def _eval_unique(ds, *, expand: bool, reranker: str | None,
                 alpha: float, top_k_for_prf: int, max_entities: int,
                 min_dominance: float, k_max: int = 10) -> dict:
    rows = []
    with tempfile.TemporaryDirectory() as tmp:
        eng = _build_engine(tmp, reranker=reranker, share_prior_alpha=alpha)
        try:
            for content, meta in ds.memories:
                clean = {k: v for k, v in meta.items()
                         if isinstance(v, (str, int, float, bool))}
                eng.remember(content, **clean)
            for q in ds.queries:
                first_pass = eng.recall(q.text, limit=k_max)
                if expand:
                    first_texts = [r.memory.content for r in first_pass]
                    expanded_q, chosen = _expand_query(
                        q.text, first_texts,
                        top_k_for_prf=top_k_for_prf,
                        max_entities=max_entities,
                        min_dominance=min_dominance,
                    )
                    results = (eng.recall(expanded_q, limit=k_max)
                               if chosen else first_pass)
                else:
                    results = first_pass
                texts = [r.memory.content for r in results]
                gold = q.expected_substrings[0].lower()
                rows.append({
                    "hit@1": int(bool(texts) and gold in texts[0].lower()),
                    "hit@5": int(any(gold in t.lower() for t in texts[:5])),
                })
        finally:
            eng.close()
    n = max(len(rows), 1)
    return {
        "hit@1": sum(r["hit@1"] for r in rows) / n,
        "hit@5": sum(r["hit@5"] for r in rows) / n,
    }


CELLS = [
    ("C0_baseline", False, None, 0.0),
    ("CP_prf_only", True, None, 0.0),
    ("CR_share_prior_only", False, "share_prior", 0.05),
    ("CB_both", True, "share_prior", 0.05),
]


def run(*, seeds, n_pairs, plain_distractors, n_facts,
        top_k_for_prf, max_entities, min_dominance) -> dict:
    t0 = time.monotonic()
    bridge_per_cell = {name: [] for name, *_ in CELLS}
    unique_per_cell = {name: [] for name, *_ in CELLS}

    for s in seeds:
        ds_b = generate_bridge_corpus(
            n_pairs=n_pairs, plain_distractors=plain_distractors, seed=s,
        )
        ds_u = generate_entity_corpus(
            n_facts=n_facts, hard_distractors_per_fact=2,
            plain_distractors=plain_distractors, seed=s + 1000,
        )
        for name, expand, reranker, alpha in CELLS:
            bridge_per_cell[name].append(_eval_bridge(
                ds_b, expand=expand, reranker=reranker, alpha=alpha,
                top_k_for_prf=top_k_for_prf, max_entities=max_entities,
                min_dominance=min_dominance,
            ))
            unique_per_cell[name].append(_eval_unique(
                ds_u, expand=expand, reranker=reranker, alpha=alpha,
                top_k_for_prf=top_k_for_prf, max_entities=max_entities,
                min_dominance=min_dominance,
            ))

    def agg(rows, keys):
        return {
            k: {
                "mean": round(statistics.mean(r[k] for r in rows), 4),
                "stdev": (round(statistics.stdev(r[k] for r in rows), 4)
                          if len(rows) > 1 else 0.0),
            }
            for k in keys
        }

    bridge_keys = ["any_hit@5", "any_hit@10", "any_hit@20",
                   "pair_recall@5", "pair_recall@10", "pair_recall@20"]
    unique_keys = ["hit@1", "hit@5"]

    return {
        "config": {
            "seeds": list(seeds), "n_pairs": n_pairs,
            "plain_distractors": plain_distractors, "n_facts": n_facts,
            "top_k_for_prf": top_k_for_prf, "max_entities": max_entities,
            "min_dominance": min_dominance,
        },
        "bridge": {
            name: agg(bridge_per_cell[name], bridge_keys)
            for name in bridge_per_cell
        },
        "unique_donoharm": {
            name: agg(unique_per_cell[name], unique_keys)
            for name in unique_per_cell
        },
        "per_seed": {
            "seeds": list(seeds),
            "bridge": {name: bridge_per_cell[name] for name in bridge_per_cell},
            "unique": {name: unique_per_cell[name] for name in unique_per_cell},
        },
        "wall_seconds": round(time.monotonic() - t0, 2),
    }


def _md(rep: dict) -> str:
    cfg = rep["config"]
    lines = [
        f"Wall: {rep['wall_seconds']}s | seeds={cfg['seeds']} "
        f"n_pairs={cfg['n_pairs']} d={cfg['min_dominance']} α=0.05 pool=20",
        "",
        "### Bridge — PRF × share_prior interaction",
        "",
        "| cell | pair@5 | pair@10 | pair@20 | any@20 |",
        "|:---|---:|---:|---:|---:|",
    ]
    for name in ["C0_baseline", "CP_prf_only",
                 "CR_share_prior_only", "CB_both"]:
        b = rep["bridge"][name]
        lines.append(
            f"| {name} | "
            f"{b['pair_recall@5']['mean']:.3f}±{b['pair_recall@5']['stdev']:.3f} | "
            f"{b['pair_recall@10']['mean']:.3f}±{b['pair_recall@10']['stdev']:.3f} | "
            f"{b['pair_recall@20']['mean']:.3f}±{b['pair_recall@20']['stdev']:.3f} | "
            f"{b['any_hit@20']['mean']:.3f}±{b['any_hit@20']['stdev']:.3f} |"
        )

    # Compute Δs vs C0 for the headline metric and the interaction term.
    c0 = rep["bridge"]["C0_baseline"]
    cp = rep["bridge"]["CP_prf_only"]
    cr = rep["bridge"]["CR_share_prior_only"]
    cb = rep["bridge"]["CB_both"]
    for k in ["pair_recall@10", "pair_recall@20"]:
        d_p = cp[k]["mean"] - c0[k]["mean"]
        d_r = cr[k]["mean"] - c0[k]["mean"]
        d_b = cb[k]["mean"] - c0[k]["mean"]
        # Interaction: actual_combined − sum_of_individuals.
        interaction = d_b - (d_p + d_r)
        lines.append(
            f"\n**{k}** Δ_PRF={d_p:+.3f} Δ_SP={d_r:+.3f} "
            f"Δ_BOTH={d_b:+.3f} → interaction={interaction:+.3f} "
            f"({'super-additive' if interaction > 0.005 else 'sub-additive' if interaction < -0.005 else 'additive'})"
        )

    lines += [
        "",
        "### Unique — do-no-harm",
        "",
        "| cell | hit@1 | hit@5 |",
        "|:---|---:|---:|",
    ]
    for name in ["C0_baseline", "CP_prf_only",
                 "CR_share_prior_only", "CB_both"]:
        u = rep["unique_donoharm"][name]
        lines.append(
            f"| {name} | "
            f"{u['hit@1']['mean']:.3f}±{u['hit@1']['stdev']:.3f} | "
            f"{u['hit@5']['mean']:.3f}±{u['hit@5']['stdev']:.3f} |"
        )
    return "\n".join(lines)


def main():
    p = argparse.ArgumentParser()
    p.add_argument("--seeds", type=str, default="17,42,101")
    p.add_argument("--n-pairs", type=int, default=60)
    p.add_argument("--plain-distractors", type=int, default=80)
    p.add_argument("--n-facts", type=int, default=80)
    p.add_argument("--top-k-for-prf", type=int, default=10)
    p.add_argument("--max-entities", type=int, default=4)
    p.add_argument("--min-dominance", type=float, default=0.3)
    p.add_argument("--out",
                   default="evals/results/prf_x_shareprior_stack.json")
    args = p.parse_args()

    seeds = [int(s) for s in args.seeds.split(",") if s.strip()]
    rep = run(
        seeds=seeds, n_pairs=args.n_pairs,
        plain_distractors=args.plain_distractors, n_facts=args.n_facts,
        top_k_for_prf=args.top_k_for_prf, max_entities=args.max_entities,
        min_dominance=args.min_dominance,
    )
    print("§5.4 stack — PRF × share_prior")
    print(_md(rep))

    if args.out:
        Path(args.out).parent.mkdir(parents=True, exist_ok=True)
        atomic_write_json(args.out, rep, default=str)
        print(f"[prf-x-shareprior] wrote {args.out}")


if __name__ == "__main__":
    main()
