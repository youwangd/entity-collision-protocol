#!/usr/bin/env python3
"""§A.4.16.3 — Paired BGE-large vs default-MiniLM CI on the LongMemEval n=100
subsample.

For each of the 100 LongMemEval-S question_ids that the BGE-large baseline
ran (`bench/results/lme_n100_k10_baseline_bge.json`), pull the matched
result from the full-500 default-embedder baseline
(`bench/results/lme_full500_k10_baseline.json`) and compute paired
bootstrap 95% CIs on Δhit@1 and Δhit@10, both overall and per
question_type.

Output: bench/results/lme_n100_bge_vs_default_ci.json
"""
...
def main() -> None:
    ...
    bge = json.loads((root / "bench/results/lme_n100_k10_baseline_bge.json").read_text())
    ...
    out: dict = {
        "n_paired": len(paired),
        "bge_artifact": "bench/results/lme_n100_k10_baseline_bge.json",
    ...
    out_path = root / "bench/results/lme_n100_bge_vs_default_ci.json"
